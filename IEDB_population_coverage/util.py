...
def get_available_allele_names(population=None, mhc_class=None):
    ...
    for locus, allele_genotype in class_population_alleles.items():
...
def get_population_list():
    ...
    for region, population_ethnicity in country_ethnicity.items():
        print("{}".format(region))
        for population, ethnicity in population_ethnicity.items():
...
def get_all_alleles():
    ...
    all_class_alleles = {}
    for mhc_class, population_map in population_coverage.items():
        all_alleles = []
        for population, locus_map in population_map.items():
...
def locus_by_allele_name(mhc_class, population, query_alleles):
    for lkey, agvalue in population_coverage.get(mhc_class).get(population).items():
